...
color 1F
	...
	::ffmpeg -hide_banner -y -i %filename% -vf scale=w=640:h=360:force_original_aspect_ratio=decrease -c:a aac -ar 48000 -c:v h264 -profile:v main -crf 20 -sc_threshold 0 -g 48 -keyint_min 48 -hls_time %duration% -hls_playlist_type vod  -b:v 800k -maxrate 856k -bufsize 1200k -b:a 96k -hls_segment_filename hls/360p_%%03d.ts hls/360p.m3u8 -vf scale=w=842:h=480:force_original_aspect_ratio=decrease -c:a aac -ar 48000 -c:v h264 -profile:v main -crf 20 -sc_threshold 0 -g 48 -keyint_min 48 -hls_time %duration% -hls_playlist_type vod -b:v 1400k -maxrate 1498k -bufsize 2100k -b:a 128k -hls_segment_filename hls/480p_%%03d.ts hls/480p.m3u8 -vf scale=w=1280:h=720:force_original_aspect_ratio=decrease -c:a aac -ar 48000 -c:v h264 -profile:v main -crf 20 -sc_threshold 0 -g 48 -keyint_min 48 -hls_time %duration% -hls_playlist_type vod -b:v 2800k -maxrate 2996k -bufsize 4200k -b:a 128k -hls_segment_filename hls/720p_%%03d.ts hls/720p.m3u8 -vf scale=w=1920:h=1080:force_original_aspect_ratio=decrease -c:a aac -ar 48000 -c:v h264 -profile:v main -crf 20 -sc_threshold 0 -g 48 -keyint_min 48 -hls_time %duration% -hls_playlist_type vod -b:v 5000k -maxrate 5350k -bufsize 7500k -b:a 192k -hls_segment_filename hls/1080p_%%03d.ts hls/1080p.m3u8

	:: parcourir tous les fichiers dans un dossier
	:: %%~xi = l'extension du fichier
	:: %%~ni = le nom du fichier sans l'extension
	:: %%~di = le lecteur sur lequel le fichier se trouve
	:: %%~pi = le chemin d'accès du fichier comme path
	:: %%~fi = le chemin d'accès complet du fichier comme volume/chemin.extention
	:: %%~zi = la taille en octet du fichier
